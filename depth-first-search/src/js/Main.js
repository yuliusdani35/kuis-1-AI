...
visBtn.addEventListener('click',()=>{
    ...
    if(algos.length===0){
        showMessage('silakan pilih algoritma untuk divisualisasikan')
        return
    }

    if(instructionInProgress){
        showMessage('Harap selesaikan instruksi terlebih dahulu, atau lewati');
        return;
    }
...
const populateToturialDiv = (algoName) =>{
    ...
    switch (algoName){    
        ...
        case "Depth-First Search":
            ...
            desc.innerText =`DFS dimulai pada node awal dan mengeksplorasi satu jalur yang mungkin hingga kedelapan mencapai node akhir atau ke titik di mana tidak ada node lain yang belum dikunjungi yang tersisa. Setelah mencapai ujung satu jalur, ia akan mundur untuk menjelajahi jalur lain yang mungkin.`;

            backFace.innerText = `Pencarian Depth-First tidak akan memberikan jalur terpendek. Itu hanya akan memberi tahu Anda jika ada jalan. Juga bukan algoritma berbobot. Anda dapat melihat jika Anda membuat beberapa sel berbobot, algoritme akan memperlakukannya sama dengan node tidak berbobot lainnya.
            DFS dapat diimplementasikan secara rekursif, atau iteratif. Dalam aplikasi ini DFS diimplementasikan secara iteratif dengan bantuan stack.`
